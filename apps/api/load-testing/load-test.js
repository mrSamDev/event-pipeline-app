...
function generatePayload(eventType) {
  ...
  switch (eventType) {
    case 'page_view':
      return { ...basePayload, url: `/page/${Math.floor(Math.random() * 100)}`, referrer: '/' };
    case 'search':
      return { ...basePayload, query: `search_term_${Math.floor(Math.random() * 1000)}`, results: 42 };
    case 'purchase':
      return { ...basePayload, amount: Math.floor(Math.random() * 10000) / 100, currency: 'USD', productId: `prod_${Math.floor(Math.random() * 500)}` };
    case 'add_to_cart':
      return { ...basePayload, productId: `prod_${Math.floor(Math.random() * 500)}`, quantity: Math.floor(Math.random() * 5) + 1 };
    case 'remove_from_cart':
      return { ...basePayload, productId: `prod_${Math.floor(Math.random() * 500)}` };
    case 'button_click':
      return { ...basePayload, buttonId: `btn_${Math.floor(Math.random() * 50)}`, label: 'Click Me' };
    case 'form_submit':
      return { ...basePayload, formId: `form_${Math.floor(Math.random() * 20)}`, fields: ['email', 'name'] };
    case 'video_play':
      return { ...basePayload, videoId: `video_${Math.floor(Math.random() * 100)}`, position: 0 };
    case 'video_pause':
      return { ...basePayload, videoId: `video_${Math.floor(Math.random() * 100)}`, position: Math.floor(Math.random() * 180) };
    default:
      return basePayload;
  }
}
