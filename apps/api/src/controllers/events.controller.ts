...
export class EventsController {
	...
	async ingestEvent(req: Request, res: Response): Promise<void> {
		try {
			...
			const rawEvents: RawEvent[] = Array.isArray(req.body)
				? req.body
				: [req.body];
			...
			if (from) {
				...
				if (!fromDate) {
					res.status(400).json({
						error: "Bad Request",
						message:
							'Invalid "from" date format. Use ISO 8601 format (e.g., 2025-01-01T00:00:00Z)',
					});
					return;
				}
			...
			if (to) {
				...
				if (!toDate) {
					res.status(400).json({
						error: "Bad Request",
						message:
							'Invalid "to" date format. Use ISO 8601 format (e.g., 2025-01-31T23:59:59Z)',
					});
					return;
				}
				options.to = toDate;
