...
- Graceful shutdown with buffer flush
  ...
  tags: [
    {
      name: "Authentication",
      description: "User authentication and session management endpoints (Better Auth)",
    },
    {
      name: "Events",
      ...
      Event: {
        ...
        properties: {
          ...
          occurredAt: {
            type: "string",
            format: "date-time",
            description: "When the event occurred (ISO 8601). Defaults to current time if not provided.",
    ...
    paths: {
      "/events": {
        post: {
          ...
          description: "Accepts single event or array of events for ingestion into the pipeline. Events are buffered in memory and flushed to MongoDB when buffer reaches 500 events or after 1000ms. Returns 202 Accepted immediately - processing is asynchronous.",
          requestBody: {
            required: true,
            content: {
              "application/json": {
                ...
                examples: {
                  singleEvent: {
                    ...
                    value: {
                      ...
                      sessionId: "sess456",
                      type: "page_view",
                      payload: { url: "/products/123", referrer: "https://google.com" },
                    },
                  },
                  batchEvents: {
                    summary: "Batch of events",
                    value: [
                      { userId: "user123", sessionId: "sess456", type: "page_view", payload: { url: "/home" } },
                      { userId: "user123", sessionId: "sess456", type: "button_click", payload: { button: "add_to_cart" } },
                    ],
                  },
                },
              },
          ...
          responses: {
              ...
              content: {
                "application/json": {
                  schema: {
                    type: "object",
                    properties: {
                      message: { type: "string", example: "Events accepted for processing" },
                      count: { type: "integer", example: 2 },
                      eventIds: { type: "array", items: { type: "string" }, example: ["uuid-1", "uuid-2"] },
                    },
                  },
                },
              },
            ...
            503: { description: "Buffer overflow - service unavailable", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } },
          },
        },
      },
      "/users/{userId}/journey": {
        get: {
          ...
          parameters: [
            ...
            { in: "query", name: "to", schema: { type: "string", format: "date-time" }, description: "End date (ISO 8601 format)", example: "2025-12-15T23:59:59Z" },
            { in: "query", name: "limit", schema: { type: "integer", minimum: 1, maximum: 1000, default: 100 }, description: "Maximum number of events to return" },
          ],
          responses: {
            200: {
              ...
              content: {
                "application/json": {
                  schema: {
                    type: "object",
                    properties: {
                      bufferSize: { type: "integer", description: "Current number of events in buffer", example: 150 },
                      isFlushingInProgress: { type: "boolean", description: "Whether a flush operation is currently running", example: false },
      ...
      "/api/auth/sign-up/email": {
        post: {
          ...
          responses: {
            ...
            400: { description: "Invalid input or email already exists", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } },
          },
        },
      },
