...
async function generateAnalyticsExport(): Promise<ExportPayload> {
  const [totalUsers, totalEvents, eventsByType, eventsByHour] = await Promise.all([
...
async function runExportJob(): Promise<void> {
  if (isJobRunning) {
    logger.warn('[DailyAnalyticsExportJob] Skipping execution - previous job still running');
    return;
  }
